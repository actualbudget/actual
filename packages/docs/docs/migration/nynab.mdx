...
:::caution
The token is only displayed once. Copy it to a safe place before closing the page. More information about the YNAB API can be found at [api.youneedabudget.com](https://api.youneedabudget.com/).
:::
...
4. In the response body, find the entry for your budget and copy its `id` value (for example, `8800b4e7-937f-4cdb-a7e7-fc35b794f85b`)

#### Step 2: Download Your Budget
...
3. Paste your budget ID into the `plan_id` field and click **Send**
...
#### Step 3: Fix the JSON File

The file from this endpoint uses a slightly different format than what Actual expects. You need to make one small edit before importing.

1. Open the downloaded JSON file in a plain text editor (Notepad on Windows, TextEdit on macOS)
2. At the very start of the file, find the text `"plan"` and change it to `"budget"`
3. Save the file

![The JSON file in a text editor, showing "plan" that must be changed to "budget"](/img/migrating/nynab-api-ui-json-edit.png)

:::warning
Skipping this step causes a **not-ynab5** error when importing. Make sure the file begins with `{"data": {"budget"` before proceeding.
:::
...
If you are comfortable with the command line, you can download your budget directly using `curl`.
...
This returns a list of your budgets. Find the `id` for the budget you want to export.
...
Replace `<ACCESS_TOKEN>` and `<BUDGET ID>` with your values.
