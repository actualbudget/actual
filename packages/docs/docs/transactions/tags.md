...
- A tag is a string of any characters (except `#` and ` `) prefixed by the `#` symbol; it can be added anywhere within the _Notes_ field of a transaction.
...
- **Add New** tags. Do not enter the `#` when adding tags here, it will be added for you. If the tag already exists, the color you pick will be reflected in the matching existing tags.
