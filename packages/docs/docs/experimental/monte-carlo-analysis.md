...
- **Assume any unspent money is spent** unticked (the default for new plans): a **Surplus cash** pot sits at the top of your pots. Everything the plan doesn't spend in a year - income beyond your spending, and anything a minimum withdrawal forced out above the plan - is saved into it at the start of the year (so it earns that year's return), and it is always the first pot drawn on when spending needs funding. It starts empty and holds cash - no access age, no tax, no fees - and its settings can't be changed in the pots list; the one thing to do with it is move money on with a contribution. Tick the setting to remove it, along with any contributions paid into it.
