---
slug: /
sidebar_position: 1
title: 'Introduction'
---

Welcome to Actual's documentation! 

## Documentation Overview

The left sidebar menu lists the main pages of the documentation. On mobile, it's the collapsed navigation menu icon at the top left. Click on a chevron to view more nested pages on a subject.

The right sidebar of each page lists the outline of the page for quick navigation to a specific subject. On mobile, it's at the top under "On this page".

An entry in the search bar will show the most useful result at the top, and all results it finds will be available.


## Where to Begin?

If you're new to Actual, consult the [roadmap for new users](/docs/getting-started/roadmap-for-new-users).

If you're new to budgeting, read about [envelope budgeting](/docs/getting-started/envelope-budgeting).

For help and support, [reach out to the maintainers and community](/contact).

## Want to Contribute?

The Actual application is actively maintained and updated by a dedicated group of contributors and maintainers who do their best to keep these documents synchronized with the changes. If you notice anything incorrect you can [open an issue in github](https://github.com/actualbudget/actual/issues/new/choose) or note it [in the `#Documentation` channel in Discord.](https://discord.com/channels/937901803608096828/1027831463103696928)

If you'd like to contribute to Actual Budget's documentation, start by reading the documentation standard, local setup, and guidelines on using images and screenshots from [here](/docs/contributing/writing-docs) and [here](https://github.com/actualbudget/actual/tree/master/packages/docs#actual-budget-community-documentation). We use Docusaurus to build our website. You can click on _Edit this page_ at the bottom left of every page to open that page's Markdown document.
