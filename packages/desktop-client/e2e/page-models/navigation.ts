...
export class Navigation {
  ...
  // Schedules has no top-level sidebar entry in the redesigned nav; it's
  // only reached via the sidebar's Transactions widget "scheduled" stat
  // (Full width, the default) or a direct link elsewhere in the app.
  async goToSchedulesPage() {
    await this.page.getByRole('button', { name: /scheduled/i }).click();

    return new SchedulesPage(this.page);
  }

  // Payees/Rules/Bank Sync are Settings tabs now, not top-level sidebar
  // entries.
  async goToRulesPage() {
