...
function AccountHeader<SheetFieldName extends SheetFields<'account'>>({
  ...
  const showChevron = hideChevron !== true;

  return (
    <Button
      variant="bare"
      aria-label={t('View {{name}} transactions', { name })}
      onPress={pressHandler ? pressHandler : () => navigate(`/accounts/${id}`)}
      style={{
        flex: 1,
        flexDirection: 'row',
        paddingTop: 15,
...
function AllAccountList({
  ...
  const [shouldScrollToClosedAccounts, setShouldScrollToClosedAccounts] =
    useState(false);
  ...
  const onToggleClosedAccounts = () => {
    ...
    if (toggledState) {
      setShouldScrollToClosedAccounts(true);
    }
  };

  useEffect(() => {
    // Make sure to scroll to the closed accounts when the user opens them,
    // otherwise it's not clear that the accounts are there.
    // Delay scroll until the component is rendered, because useEffect()
    // might trigger before that.
    if (shouldScrollToClosedAccounts && showClosedAccounts) {
      setShouldScrollToClosedAccounts(false);
      setTimeout(() => {
        closedAccountsRef.current?.scrollIntoView({ behavior: 'smooth' });
      });
    }
  }, [shouldScrollToClosedAccounts, showClosedAccounts]);
      ...
      <PullToRefresh onRefresh={onSync}>
        <View aria-label={t('Account list')} style={{ margin: 10 }}>
          ...
          {closedAccounts.length > 0 && (
            <AccountHeader
              id="closed"
              name={showClosedAccounts ? t('Closed') : t('Closed...')}
              pressHandler={onToggleClosedAccounts}
