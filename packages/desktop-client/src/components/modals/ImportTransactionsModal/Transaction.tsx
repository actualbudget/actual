...
function resolveCategoryDisplay(
  ...
  return { categoryName, isNewCategory };
}

type CategoryDisplayProps = {
  resolved: CategoryResolved | null;
  t: (key: string) => string;
};

function CategoryDisplay({ resolved, t }: CategoryDisplayProps): ReactNode {
  if (!resolved) {
    return null;
  }

  const { categoryName, isNewCategory } = resolved;
...
export function Transaction({
  ...
  const categoryResolved = resolveCategoryDisplay(
    transaction.category,
    categoryMap,
  );
  const categoryFieldTitle =
    categoryResolved === null
      ? undefined
  ...
  return (
    ...
    >
      ...
      <Field width="flex" title={categoryFieldTitle}>
        <CategoryDisplay resolved={categoryResolved} t={t} />
      </Field>
      {inOutMode && (
        <Field
          width={90}
