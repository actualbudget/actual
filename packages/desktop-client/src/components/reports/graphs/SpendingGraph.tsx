...
type CustomTooltipProps = {
  ...
  compare: string;
  format?: (value: unknown, formatType: FormatType) => string;
};
          ...
          <div style={{ lineHeight: 1.5 }}>
            ...
            {['cumulative'].includes(balanceTypeOp) && (
              <AlignedText
                ...
                right={format(Math.round(comparison || 0), 'financial')}
              />
            )}
            {payload[0].payload.months[compare]?.cumulative ? (
              <AlignedText
...
export function SpendingGraph({
  ...
  const tickFormatter: ComponentProps<typeof YAxis>['tickFormatter'] = tick => {
    if (!privacyMode) return `${format(tick, 'financial')}`; // Formats the tick values as strings with commas
    return '...';
  };
              ...
              <AreaChart
                ...
                margin={{
                  ...
                  left: computePadding(
                    ...
                    (value: number) => format(value, 'financial'),
                  ),
                  bottom: 0,
                }}
              >
