...
export function RecurringPaymentsCard({
  ...
  const showExpenses =
    savedSettings.showExpenses !== undefined
      ? savedSettings.showExpenses
      : true;
  const showIncome =
    savedSettings.showIncome !== undefined ? savedSettings.showIncome : false;
  const minConfidence = savedSettings.minConfidence || 40;
  const maxVariation =
    savedSettings.maxVariation !== undefined ? savedSettings.maxVariation : 100;
      ...
      payeeGroups.forEach((transactionList, key) => {
        ...
        if (confidenceScore >= minConfidence && averageDaysBetween <= 400) {
          ...
          recurringList.push({
            payee: payeeName,
            account:
              accounts.find(acc => acc.id === lastTransaction.account)?.name ||
              'Unknown Account',
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            {minConfidence}%+ • ±{maxVariation}%
          </div>
          {selectedAccounts.length > 0 && (
            <div style={{ color: '#007bff' }}>
              {selectedAccounts.length === 1
                ? (
                    accounts.find(acc => acc.id === selectedAccounts[0])
                      ?.name || 'Account'
                  ).slice(0, 8)
                ...
                >
                  ...
                  <div
                    style={{
                      display: 'flex',
                      alignItems:
                        'flex-end' /* Changed to flex-end to make bars start at the same level */,
                  ...
                  >
                    {recurringPayments.slice(0, 8).map((payment, i) => {
                      const barHeight = Math.max(
                        (payment.averageAmount / maxAmount) * 50,
                        8,
                      ); // Reduced max height to 50px
                          ...
                          >
                            <div
                              style={{
                                width: '100%',
                                height: `${barHeight}px`,
                                backgroundColor: payment.isExpense
                                  ? '#87dee8'
                                  : '#28a745',
