...
class PluginWidgetErrorBoundary extends React.Component<
    ...
    switch (reason) {
      case 'not-found':
        return meta?.pluginId 
          ? t('Plugin "{{pluginId}}" widget "{{widgetType}}" is not available', {
              pluginId: meta.pluginId,
              widgetType: meta.pluginWidgetType || 'unknown'
            })
          : t('Widget configuration is invalid');
      case 'error':
        return t('The plugin widget encountered an error and cannot be displayed');
      case 'loading-failed':
        return t('The plugin could not be loaded. It may be disabled or have an error.');
      default:
      ...
      >
        ...
        {isEditing && (
          <Text style={{ 
            ...
            {t('You can remove this widget using the menu')}
          </Text>
        )}
      </View>
    </ReportCard>
