...
class ColorBar {
    ...
    constructor(color: string = ColorDefinitions.UnderBudgetRemaining, width: number = 50, category: string = "", rawValue: string = "") {
        this.color = color;
        this.width = width;
...
function getColorBars(budgeted: number, spent: number, balance: number, goal: number, isLongGoal: boolean) {
...
export function CategoryVisual({
    ...
    const [leftBar, setLeftBar] = React.useState<ColorBar>(new ColorBar())
    const [rightBar, setRightBar] = React.useState<ColorBar>(new ColorBar())

    // The budgeted amount for this month
    const budgeted = useSheetValue<'envelope-budget', 'budget'>(envelopeBudget.catBudgeted(category.id));
    // The amount spent this month
    const spent = useSheetValue<'envelope-budget', 'sum-amount'>(envelopeBudget.catSumAmount(category.id));
    /* Goal is either the template value or the goal value, so use it in conjunction with long-goal. */
    const goal = useSheetValue<'envelope-budget', 'goal'>(envelopeBudget.catGoal(category.id));
    // If a #goal for the category exists.
    const isLongGoal: boolean = useSheetValue<'envelope-budget', 'long-goal'>(envelopeBudget.catLongGoal(category.id)) > 0;
    // The current category balance based on the budgeted, spent, and previous balance amounts
    const balance = useSheetValue<'envelope-budget', 'leftover'>(envelopeBudget.catBalance(category.id));

    useEffect(() => {
        const setColorBars = async () => {
            setTimeout(() => {
                const [freshLeftBar, freshRightBar] = getColorBars(budgeted, spent, balance, goal, isLongGoal);
