...
it('hides the hold action when future assignments are included', () => {
  vi.mocked(useSyncedPref).mockReturnValue(['include-future', vi.fn()]);
  vi.mocked(useEnvelopeSheetValue).mockImplementation(binding => {
    if (binding === envelopeBudget.toBudgetWithFuture) {
      return 100;
    }
    return 0;
  });
  ...
  expect(useEnvelopeSheetValue).toHaveBeenCalledWith(
    envelopeBudget.toBudgetWithFuture,
  );
  expect(screen.getByText('Move to a category')).toBeInTheDocument();
  expect(screen.queryByText('Hold for next month')).not.toBeInTheDocument();
});
