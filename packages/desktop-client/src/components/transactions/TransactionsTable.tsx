...
function StatusCell({
  ...
  const isClearedField = status === "cleared" || status === "reconciled" ||
    status == null;
...
const Transaction = memo(function Transaction({
  ...
  const [showReconciliationWarning, setShowReconciliationWarning] = useState(
    false,
  );
  ...
  const onUpdate: TransactionUpdateFunction = async (name, value) => {
    ...
    if (transaction[name] !== value) {
      const isReconciledField = name === "credit" ||
        name === "debit" ||
        name === "payee" ||
        name === "account" ||
        name === "date";
  ...
  const safeOnDrop: OnDropCallback | undefined = isPreview
    ? undefined
    : isChildTransaction
      ? isSiblingDrag ? onDrop : undefined
      : onDrop;
  ...
  const isBeingDragged = draggedId != null &&
      ...
      between normal Date and Payee or Account and Payee if needed */
            ...
            ? (
              <InputCell
                ...
                value={isOffBudget
                  ? t("Off budget")
                  : isBudgetTransfer
                    ? categoryId != null ? t("Needs Repair") : t("Transfer")
                    : ""}
                valueStyle={valueStyle}
                style={{
                  fontStyle: "italic",
        ...
        {showBalance && (
          <Cell
            ...
            name="balance"
            value={runningBalance == null || isChild || isTemporaryId(id)
              ? ""
          ...
          >
            ...
            >
              {isParent
                ? (
                  <Trans>
                    Split ({{ count: subtransactions?.length ?? 0 }})
                  </Trans>
                )
                : categoryId
                  ? (
...
function NotesCell({
  ...
  function onSelect(
    ...
    if (option) {
      const newValue = value.slice(0, start) + option.name +
        value.slice(end + 1);
      setTimeout(() => {
  ...
  function onKeyDown(e: KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "Tab" && highlightedEntries.length) {
      const selectedId = highlightedEntries.item(0).getAttribute(
        "data-tag-option-id",
      ) ?? "";
      e.preventDefault();
      e.stopPropagation();
                ...
                >
                  {items.map((item, idx) => (
                    <div
                      ...
                      data-tag-highlighted={highlightedIndex === idx
                        ? "true"
                        : undefined}
                      className={cx(css({
    ...
    >
      {transactions.map((transaction) => (
        <Transaction
          ...
          focusedField={editingTransaction === transaction.id
            ? focusedField
            : undefined}
...
function TransactionTableInner({
  ...
  const renderRow: TableProps<TransactionEntity>["renderItem"] = ({
    ...
    const siblingCount = trans.is_child && trans.parent_id
        ...
        {props.isAdding && (
          ...
          >
            <NewTransaction
              ...
              transferAccountsByTransaction={props
                .transferAccountsByTransaction}
      ...
      if (e.key === "Enter") {
        if (e.metaKey || e.ctrlKey) {
          ...
          afterSave(() => {
            ...
            const isSplit = lastTransaction.parent_id ||
              lastTransaction.is_parent;
    ...
    const onSplit = useMemo(() => {
      return (id: TransactionEntity["id"]) => {
        if (isTemporaryId(id)) {
          ...
          const trans = latestState.current.transactions.find((t) =>
            t.id === id
          );
    ...
    const onDistributeRemainder = useCallback(
      async (id: TransactionEntity["id"]) => {
        ...
        if (emptyTransactions.length > 0) {
          ...
          let remainingCents = remainingAmount -
            amountPerTransaction * emptyTransactions.length;
          ...
          for (
            ...
            amounts[transactionIndex] = Math.floor(
              siblingProportions[transactionIndex] * remainingAmount,
            ) + transaction.amount;
          }

          let remainingCents = parentTransaction.amount -
            amounts.reduce((acc, a) => acc + a, 0);
