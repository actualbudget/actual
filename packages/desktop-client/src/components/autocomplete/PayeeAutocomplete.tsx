...
function PayeeList({
  ...
  const { newPayee, nearbyPayees, suggestedPayees, payees, transferPayees } = useMemo(() => {
    let currentIndex = 0;
...
export function PayeeAutocomplete({
  ...
  const retrievedNearbyPayees = locationAccess
    ? useNearbyPayees() || []
    : [];
  if (!payees) {
...
function NearbyPayeeItem({
  ...
  const narrowStyle = locationAccess
    ...
    : {};
  const iconSize = locationAccess ? 14 : 8;
  let paddingLeftOverFromIcon = 20;
  let itemIcon = undefined;
  if (item.favorite) {
    itemIcon = (
  ...
  const distanceText =
    distance !== undefined
      ? locationService.formatDistance(distance, useImperial)
      : '';

  const handleForgetClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
