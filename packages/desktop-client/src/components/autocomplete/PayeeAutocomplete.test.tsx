...
function extractPayeesAndHeaderNames(screen) {
    return [ ...screen.getByTestId('autocomplete').querySelectorAll(`${PAYEE_SELECTOR}, ${PAYEE_SECTION_SELECTOR}`)]
...
describe('PayeeAutocomplete.getPayeeSuggestions', () => {
    ...
    test('list with less than the maximum favorites adds common payees', async () => {
        ...
        vi.mocked(useCommonPayees).mockReturnValue([
            makePayee("Bruce"), makePayee("Natasha"), makePayee("Steve"), makePayee("Tony"), makePayee("Carol")
        ]);
        const expectedPayeeOrder = [
            'Suggested Payees',
            "Eve", "Bruce", "Natasha", "Steve", "Tony", 
            'Payees',
            "Alice", "Bob", "Carol"];
        ...
        expect(
            [ ...screen.getByTestId('autocomplete').querySelectorAll(`${PAYEE_SELECTOR}, ${PAYEE_SECTION_SELECTOR}`)]
            .map(e => e.getAttribute('data-testid'))
            .flatMap(id => id.split('-', 1))
    ...
    test('list with more than the maximum favorites only lists favorites', async () => {
        ...
        const expectedPayeeOrder = [
            'Suggested Payees',
            "Alice", "Bob", "Bruce", "Carol", "Eve", "Tony",
            'Payees',
            "Natasha", "Steve",
        ];
        const autocomplete = renderPayeeAutocomplete({ payees });
        ...
        expect(extractPayeesAndHeaderNames(screen))
            .toStrictEqual(expectedPayeeOrder);
    })
    ...
    test('list with no favorites shows just the payees list', async () => {
        //Note that the payees list assumes the payees are already sorted
        const payees = [
            makePayee("Alice"), makePayee("Bob"), makePayee("Eve"), makePayee("Natasha"), makePayee("Steve")
        ]
        ...
        expect(
            [ ...screen.getByTestId('autocomplete').querySelectorAll('[data-testid][role=option]')]
            .map(e => e.getAttribute('data-testid'))
            .flatMap(id => id.split('-', 1))
        ).toStrictEqual(expectedPayeeOrder);
        expect(
             [ ...screen.getByTestId('autocomplete').querySelectorAll('[data-testid$="-item-group"]')]
            .map(e => e.getAttribute('data-testid'))
            .flatMap(id => id.split('-', 1))
