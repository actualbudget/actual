...
function format(
  ...
  effectiveNaturalDecimalPlaces: number,
): string {
  switch (type) {
    case 'string': {
    ...
    case 'financial': {
      ...
      if (localValue == null || localValue === '' || localValue === 0) {
        return integerToCurrency(0, formatter, effectiveNaturalDecimalPlaces);
...
export function useFormat(): UseFormatResult {
  ...
  const effectiveNaturalDecimalPlaces = activeCurrency.decimalPlaces;
