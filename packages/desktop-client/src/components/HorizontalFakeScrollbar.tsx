...
export const HorizontalFakeScrollbar: FC<HorizontalFakeScrollbarProps> = ({
  maxScroll,
  clientWidth,
}) => {
  ...
  const [maxPosition, setMaxPosition] = useState(clientWidth - (clientWidth / maxScroll) * clientWidth); // Initialize maxPosition
  ...
  const handleContainerClick = (event: MouseEvent<HTMLDivElement>) => {
    ...
    const newLeft = clickX > currentLeft ? Math.min(
      Math.max(clickX - sliderWidth, 0),
      maxPosition
    ) : Math.min(
        Math.max(clickX, 0),
        maxPosition
  ...
  return sliderWidth < clientWidth && <div
      style={{
        position: 'absolute',
        bottom: '0',
        left: '0',
