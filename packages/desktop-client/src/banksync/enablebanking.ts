...
import { AppDispatch } from "@desktop-client/redux/store";
import { unlink } from "fs";
import { AccountEntity, SyncServerGoCardlessAccount } from "loot-core/types/models";
import { EnableBankingToken } from "loot-core/types/models/enablebanking";
import { Action, Dispatch } from "redux";


export function selectEnableBankingAccounts(dispatch:AppDispatch, token:EnableBankingToken, accountEntity?:AccountEntity){
    ...
    if(accountEntity && accountEntity.official_name){
      ...
      const account = accounts.filter(tokenAccount => tokenAccount.official_name == accountEntity.official_name).at(0);
      if(account){
...
export function authorizeEnableBankingSession(dispatch:AppDispatch, account?:AccountEntity, onUnlink?:()=>void){
