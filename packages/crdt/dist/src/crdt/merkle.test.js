...
function message(timestamp, hash) {
    timestamp = timestamp_1.Timestamp.parse(timestamp);
    timestamp.hash = () => hash;
    return { timestamp };
}
...
describe('merkle trie', () => {
    test('adding an item works', () => {
        let trie = merkle.insert({}, timestamp_1.Timestamp.parse('2018-11-12T13:21:40.122Z-0000-0123456789ABCDEF'));
    ...
    test('diffing works with empty tries', () => {
        let trie1 = {};
        let trie2 = merkle.insert({}, timestamp_1.Timestamp.parse('2009-01-02T10:17:37.789Z-0000-0000testinguuid1'));
    ...
    test('diffing differently shaped tries returns correct time', () => {
        ...
        // empty trie
        expect(new Date(merkle.diff({}, trie)).toISOString()).toBe('1970-01-01T00:00:00.000Z');
        expect(new Date(merkle.diff(trie, {})).toISOString()).toBe('1970-01-01T00:00:00.000Z');
