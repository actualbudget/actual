...
export type PluginContext = Omit<HostContext, 'registerMenu' | 'pushModal' | 'registerRoute' | 'registerDashboardWidget'> & {
  registerMenu: (
...
type HostRenderCallback = (container: HTMLDivElement) => void | (() => void);
...
export interface HostContext {
  ...
  pushModal: (
    parameter: HostRenderCallback,
    modalProps?: BasicModalProps,
  ) => void;
  popModal: () => void;

  registerRoute: (path: string, routeElement: HostRenderCallback) => string;
  ...
  registerDashboardWidget: (
    widgetType: string,
    displayName: string,
    renderWidget: HostRenderCallback,
    options?: {
      defaultWidth?: number;
      defaultHeight?: number;
      minWidth?: number;
