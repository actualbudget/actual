...
-- Drop and recreate v_transactions_layer2 view
...
SELECT
  ...
  COALESCE(t.amount, 0) AS amount,  -- Use COALESCE instead of IFNULL for PostgreSQL compatibility
  t.notes AS notes,
  t.date AS date,
...
-- Create index on the transactions table
CREATE INDEX trans_sorted ON transactions(date DESC, starting_balance_flag, sort_order DESC, id);

-- Drop and recreate v_transactions_layer1 view
DROP VIEW IF EXISTS v_transactions_layer1;
CREATE VIEW v_transactions_layer1 AS
SELECT t.* 
FROM v_transactions_layer2 t
LEFT JOIN transactions t2 ON (t.is_child IS TRUE AND t2.id = t.parent_id)
WHERE COALESCE(t.tombstone, FALSE) IS FALSE AND COALESCE(t2.tombstone, FALSE) IS FALSE;  -- Use COALESCE instead of IFNULL

-- Drop and recreate v_transactions view
DROP VIEW IF EXISTS v_transactions;
CREATE VIEW v_transactions AS
SELECT t.* 
FROM v_transactions_layer1 t
ORDER BY t.date DESC, t.starting_balance_flag, t.sort_order DESC, t.id;

-- Drop and recreate v_categories view
...
SELECT
  ...
  name,
  is_income,
  cat_group AS "group",  -- "group" is a reserved keyword in PostgreSQL, so we alias it
  sort_order,
  tombstone
FROM categories;
