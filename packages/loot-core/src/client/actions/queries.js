...
import {
  ...
  addWrongAccountNameNotification
} from './notifications';
...
export function updateAccount(account) {
  return async function(dispatch) {
    const error = await send('account-update', account);

    switch (error) {
      case 'Account name already exists':
        dispatch(addWrongAccountNameNotification(account.name));
        break;
      default:
        // No error, so we can update the visible name of the account.
...
export function createAccount(name, type, balance, offBudget) {
  return async function(dispatch) {
    ...
    switch (id) {
      case 'Account name already exists':
        dispatch(addWrongAccountNameNotification(name));
        return null;
