...
class categoryTemplate {
  ...
  private set priorities(value) {
    this.priorities = value;
  }
  private set remainderWeight(value) {
    this.remainderWeight = value;
  }
  private set originalBudget(value) {
  ...
  constructor(templates, categoryID: string, month: string) {
    ...
    this.templates = templates.filter(t => {
      t.directive === 'template' && t.type != 'remainder';
    });
    this.remainder = templates.filter(t => {
      t.directive === 'template' && t.type === 'remainder';
    });
    this.goals = templates.filter(t => {
      t.directive === 'goal';
    });
