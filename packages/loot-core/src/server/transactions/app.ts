...
app.method(
  ...
  mutator(async function (transaction) {
    await app.handlers['transactions-batch-update'].call(this, {
      added: [transaction],
    });
    return {};
  }),
...
app.method(
  'transactions-export',
  mutator(async function ({ transactions, accounts, categoryGroups, payees }) {
...
app.method(
  'transactions-import',
  mutator(function ({ accountId, transactions, isPreview }) {
    return withUndo(async () => {
