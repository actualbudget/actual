...
app.method(
  ...
  mutator(async function (transaction) {
    await app.handlers['transactions-batch-update'].call(this, { added: [transaction] });
    return {};
  }
  )
);
...
app.method(
  'transaction-update',
  mutator(async function (transaction) {
    await app.handlers['transactions-batch-update'].call(this, { updated: [transaction] });
    return {};
  }
  )
);
...
app.method(
  'transaction-delete',
  mutator(async function (transaction) {
    await app.handlers['transactions-batch-update'].call(this, { deleted: [transaction] });
    return {};
  }
  )
);
