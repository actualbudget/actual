...
import * as sqliteDb from '../../../server/db';
import { actualSchema, excluded } from '../../../server/db/schema';
...
async function insertOrUpdateRow(
  ...
  // Get from sqlite.
  const row = await sqliteDb.first<Record<string, unknown>>(
  ...
  const updateValues = sql.join(
    columnNames
      ...
      .map(column => sql`${sql.identifier(column)} = ${excluded(column)}`),
    sql`,`,
  );
