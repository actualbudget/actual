  ...
  worker.onmessage = event => {
    ...
    if (msg.type === 'connect') {
      ...
      if (msg.message && msg.message.includes('indexeddb-quota-error')) {
        alert(
          'We hit a limit on the local storage available. Edits may not be saved. Please get in touch https://docs.actualbudget.org/Contact/ so we can help debug this.',
        );
      }
