...
// TODO: Remove default value for currencyNaturalDecimalPlaces and update all call sites.
export function integerToCurrency(
  integerAmount: IntegerAmount,
  formatter = getNumberFormat().formatter,
  currencyNaturalDecimalPlaces: number = 2,
) {
  const divisor = Math.pow(10, currencyNaturalDecimalPlaces);
...
export function amountToInteger(
  ...
  const multiplier = Math.pow(10, currencyNaturalDecimalPlaces);
