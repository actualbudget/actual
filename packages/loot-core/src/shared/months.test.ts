...
describe('pay period integration', () => {
  ...
  test('end-to-end pay period integration', () => {
    ...
    const displayName = monthUtils.getMonthDisplayName(payPeriodMonth, payPeriodConfig);
    expect(displayName).toMatch(/Jan-\d+/); // Should be "Jan-1" or similar
    
    const dateRange = monthUtils.getMonthDateRange(payPeriodMonth, payPeriodConfig);
