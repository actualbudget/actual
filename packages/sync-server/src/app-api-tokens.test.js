...
const deleteUser = userId => {
  ...
  getAccountDb().mutate('DELETE FROM files WHERE owner = ?', [userId]);
...
describe('/api-tokens', () => {
  let userId, sessionToken, defaultBudgetId;

  beforeEach(() => {
    userId = uuidv4();
    sessionToken = generateSessionToken();
    defaultBudgetId = uuidv4();
    createUser(userId, 'testuser');
    createSession(userId, sessionToken);
    createFile(defaultBudgetId, userId);
  });
  ...
  describe('POST /', () => {
    it('should create a new API token', async () => {
      const res = await request(app)
        .post('/')
        .send({ name: 'Test Token', budgetIds: [defaultBudgetId] })
    ...
    it('should return 400 if name is longer than 256 characters', async () => {
      const res = await request(app)
        .post('/')
        .send({ name: 'a'.repeat(257), budgetIds: [defaultBudgetId] })
        .set('x-actual-token', sessionToken);
    ...
    it('should return 400 if budgetIds is empty', async () => {
      const res = await request(app)
        .post('/')
        .send({ name: 'Test Token', budgetIds: [] })
        .set('x-actual-token', sessionToken);

      expect(res.statusCode).toEqual(400);
      expect(res.body.status).toBe('error');
      expect(res.body.reason).toBe('empty-budget-ids');
    });

    it('should return 400 if budgetIds is missing', async () => {
      const res = await request(app)
        .post('/')
    ...
    it('should return 400 if expiresAt is in the past', async () => {
      ...
      expect(res.statusCode).toEqual(400);
      expect(res.body.status).toBe('error');
      expect(res.body.reason).toBe('invalid-expires-at');
    });

    it('should create token with expiresAt in the future', async () => {
      ...
      expect(res.statusCode).toEqual(200);
      expect(res.body.status).toBe('ok');
      expect(res.body.data.expiresAt).toBe(futureTimestamp);
    });
  ...
  describe('GET /', () => {
    it('should list user tokens', async () => {
      ...
      await request(app)
        .post('/')
        .send({ name: 'Token 1', budgetIds: [defaultBudgetId] })
        .set('x-actual-token', sessionToken);

      await request(app)
        .post('/')
        .send({ name: 'Token 2', budgetIds: [defaultBudgetId] })
        .set('x-actual-token', sessionToken);
    ...
    it('should only return tokens for the current user', async () => {
      ...
      await request(app)
        .post('/')
        .send({ name: 'User 1 Token', budgetIds: [defaultBudgetId] })
        .set('x-actual-token', sessionToken);
      ...
      createFile(budgetId2, userId2);

      // Create token for second user
      await request(app)
        .post('/')
        .send({ name: 'User 2 Token', budgetIds: [budgetId2] })
  ...
  describe('DELETE /:id', () => {
    it('should revoke a token', async () => {
      ...
      const createRes = await request(app)
        .post('/')
        .send({ name: 'Token to Delete', budgetIds: [defaultBudgetId] })
        .set('x-actual-token', sessionToken);
  ...
  describe('PATCH /:id', () => {
    it('should enable/disable a token', async () => {
      ...
      const createRes = await request(app)
        .post('/')
        .send({ name: 'Token to Toggle', budgetIds: [defaultBudgetId] })
        .set('x-actual-token', sessionToken);
