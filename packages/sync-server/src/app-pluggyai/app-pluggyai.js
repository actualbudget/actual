  ...
  handleError(async (req, res) => {
    try {
      ...
      itemIds.forEach((item, index) => {
        ...
        console.log(`  ${index + 1}. "${item}" - Valid UUID: ${isValid} (Length: ${item.length})`);
      });
      ...
      for (const item of validItemIds) {
        ...
        console.log(`Accounts fetched for item ${item}:`, partial.results.map(acc => ({ id: acc.id, name: acc.name, type: acc.type })));
        ...
        if (isValidUUID(item)) {
          try {
            ...
            console.log(`Raw investments data for item ${item}:`, JSON.stringify(investments.results, null, 2));
            console.log(`Investments fetched for item ${item}:`, investments.results.map(inv => ({ 
              id: inv.id, 
              name: inv.name, 
              type: 'INVESTMENT',
              balance: inv.balance,
            ...
            if (investments.results.length > 0) {
              ...
              console.log(`Consolidated investments for item ${item}:`, consolidatedInvestments.map(acc => ({
                id: acc.id,
                name: acc.name,
                balance: acc.balance,
            ...
            console.log(`No investments found for item ${item}:`, investmentError.message);
          }
        } else {
          console.log(`⚠️  Skipping investments for itemId "${item}" - not a valid UUID format`);
  ...
  handleError(async (req, res) => {
    ...
    try {
      ...
      if (accountId.startsWith('investment-')) {
        ...
        console.log('ItemId is valid UUID:', /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(itemId));
        ...
        if (targetAccount) {
          ...
          for (const investment of targetAccount.investmentData.investments) {
            try {
              ...
              console.log(`Error fetching transactions for investment ${investment.id}:`, error.message);
            }
          }
  ...
  investments.forEach((investment, index) => {
    ...
    if (investment.balance !== undefined && investment.balance !== null) {
      ...
      if (isNaN(investmentBalance)) {
        console.log(`Invalid balance for investment ${investment.id}: ${investment.balance}`);
        investmentBalance = 0;
      }
    }
    ...
    console.log(`Added investment ${investment.id}. Balance: ${investmentBalance}, Total so far: ${totalBalance}`);
  });
