...
const StartAuthRequestBodySchema = v.looseObject({
  aspsp: v.looseObject({ name: v.string(), country: v.string() }),
  redirect_url: v.string(),
  state: v.string(),
  access: v.looseObject({ valid_until: v.string() }),
});
