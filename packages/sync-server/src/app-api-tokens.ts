...
// API tokens must never manage API tokens; enforced for every route below.
  ...
  async (req: Request<object, object, CreateTokenBody>, res: Response) => {
    ...
    if (name.trim().length > 256) {
      res.status(400).send({
        status: 'error',
        reason: 'invalid-name',
        details: 'Token name must be 256 characters or fewer',
      });
      return;
    }
    ...
    if (budgetIds.length === 0) {
      res.status(400).send({
        status: 'error',
        reason: 'empty-budget-ids',
        details: 'At least one budget must be selected',
      });
      return;
    }
    ...
    if (
      ...
      res.status(400).send({
        ...
        details:
          'expiresAt must be a unix timestamp in the future, or null for no expiration',
      });
      return;
    }
