...
import * as fetchClient from '../messaging';
import { enableGlobalMutations, disableGlobalMutations } from '../mutators';
import { setServer } from '../server-config';
...
jest.mock('../migrate/migrations', () => {
  const realMigrations = jest.requireActual('../migrate/migrations');
  return {
    ...realMigrations,
    migrate: async db => {
      _id = 100_000_000;
...
global.emptyDatabase = function (avoidUpdate) {
  return async () => {
    ...
    sqlite.execQuery(
      memoryDB,
      nativeFs.readFileSync(__dirname + '../sql/init.sql', 'utf8'),
    );
